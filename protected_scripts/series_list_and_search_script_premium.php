...
if(isset($_POST["Req_Type"]))
{
	...
	if($_POST["Req_Type"]=="simple_search") 
	{
		...
		$query=$query."SELECT * FROM `Main_Premium` WHERE MATCH(`Indeksi_per_Kerkim`, `Tituj_te_Tjere`, `Nenkategorite`, `Viti1`, `Viti2`, `Regjia`, `Aktoret`) AGAINST('$search_string' IN BOOLEAN MODE)";
	...
	else if($_POST["Req_Type"]=="advanced_search") 
	{
		...
		$query=$query."SELECT * FROM `Main_Premium` WHERE";
	...
	else if($_POST["Req_Type"]=="subcategories_filter") 
	{
		...
		$query=$query."SELECT * FROM `Main_Premium` WHERE `Kategoria`='$Kategoria' AND (`Nenkategorite` ";
...
//Nje kerkese normale pa asnje flamur. (Per kete mjafton te jete percaktuar vetem Kategoria dhe dritarja.
	...
	$query="SELECT * FROM `Main_Premium` WHERE `Kategoria`='".$Kategoria."' ORDER BY `".$Sort_Col."` ".$Sort_ASC_DESC; 
	if(isset($Sort_Col2) AND isset($Sort_ASC_DESC2)) $query=$query.", `".$Sort_Col2."` ".$Sort_ASC_DESC2;
	$result=mysqli_query($VID_SERIES, $query);
}
...
{	
	...
	else if($i>=(($Window-1)*$Window_Size) AND $i<=($Window*$Window_Size-1))
	{
		...
		echo '<li>';
		echo '<a href="/premium/members/seriale_shqip/skeda/'.$row["Indeksi"].$row["Dubluar_Titruar"].'" target="_blank">';
