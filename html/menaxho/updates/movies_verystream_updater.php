...
mysqli_query($VID_MOVIES,"UPDATE `$movies_table` SET `Stream_3` =  '' ");	
...
for($i=0; $i<count($data_array); $i++)
{
	...
	$Stream_3=$data_array[$i][1];
	//$data_array structure------------>> Indeksi,Stream_2
	//First query per futjen e episodeve
	$query="UPDATE `$movies_table` SET `Stream_3` =  '$Stream_3' WHERE  `Indeksi` = '$Indeksi' ";
	//Kodi per pjesen
	mysqli_query($VID_MOVIES,$query);	
}

$result=mysqli_query($VID_MOVIES,"SELECT * FROM `$movies_table` WHERE `Stream_3` = '' ORDER BY `Indeksi` ASC ");
echo "<br><br>FILMAT ME MUNGESE NE $movies_table NE VERYSTREAM";
