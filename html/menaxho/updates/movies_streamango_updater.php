...
$file_name="movies_streamango_links.txt";	//Emri i file-it
...
while (!feof($file_handle))
{
	...
	if(!empty($rreshti))
	{
		...
		$delimiter=strrpos($raw_streamlink,"_mp4"); 	//finds the pos of the '-' to 
		//extract the index
...
mysqli_query($VID_MOVIES,"UPDATE `$movies_table` SET `Stream_2` =  '' ");	

/*----------------------------Creating and Executing the Queries-----------------------------------*/
for($i=0; $i<count($data_array); $i++)
{
	$Indeksi=$data_array[$i][0];
	$Stream_2=$data_array[$i][1];
	//$data_array structure------------>> Indeksi,Stream_2
	//First query per futjen e episodeve
	$query="UPDATE `$movies_table` SET `Stream_2` =  '$Stream_2' WHERE  `Indeksi` = '$Indeksi' ";
	//Kodi per pjesen
	mysqli_query($VID_MOVIES,$query);	
}

$result=mysqli_query($VID_MOVIES,"SELECT * FROM `$movies_table` WHERE `Stream_2` = '' ORDER BY `Indeksi` ASC ");
echo "<br><br>FILMAT ME MUNGESE NE $movies_table NE STREAMANGO";
