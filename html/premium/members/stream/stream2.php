...
<html lang="sq">
	...
	<?php
		...
		<link rel="stylesheet" type="text/css" href="stream_style_2.css">
	...
	<?php if(login_check($mysqli) == true): ?>	    
		...
		if(strpos($referer, "videotekaime.net") OR strpos($referer, "localhost")) 
		{
			if(isset($_GET["SoF"]))
			{
		        ...
		        if($SoF=="F")
		        {
		        	...
		        	$Streamlink=str_replace("/view", "/preview", $inner_row["Stream_3"]);
		        }
		        //Serial
		        else if($SoF=="S")
		        {	
		        	...
		        	//Creating the Streamlink
              $inner_query="SELECT * FROM `$Indeksi` WHERE `Episodi`='$Episodi'";
              //Ndrysho gjerat nese Seriali eshte me pjese.
              if($Pjesa!="") $inner_query=$inner_query." AND `Pjesa`='$Pjesa'";
              ...
              $inner_row=mysqli_fetch_assoc(mysqli_query($VID_SERIES, $inner_query));
	        	...
	        	if($rightToWatch>0)
		        {
			         include_once("stream_nav_premium.php");
